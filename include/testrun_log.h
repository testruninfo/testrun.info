 ...
 */
static inline char *testrun_log_timestamp(bool micro) {
...
#define testrun_log_file_print_time_json(file, level, timestamp, fmt, ...) \
        fprintf(file, "{\"timestamp\":\"%s\","\
                        "\"level\":%d," \
                        "\t\"message\":\"" fmt "\"\n}",\
                        timestamp, level, fmt, ##__VA_ARGS__)
...
#define testrun_log_file(level, file, ...) \
        testrun_log_file_print_all_json_format2(file, level, \
                testrun_log_timestamp(true), __VA_ARGS__)
/*
#define testrun_log_file(level, file, ...) \
        testrun_log_file_print_level_plain(file, level, __VA_ARGS__)
*/
/*      ------------------------------------------------------------------------
 *
 *      MESSAGE ROUTER
